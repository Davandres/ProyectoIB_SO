#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
#include <time.h>
#include <unistd.h>
#include <string.h>

#define NUM_HILOS 2
#define LECTURAS_POR_SENSOR 10000  // Cada sensor genera 1000 lecturas

// Tipos de sensores
typedef enum {
    TEMPERATURA,
    HUMEDAD,
    RUIDO,
    LUMINOSIDAD
} TipoSensor;

// Estructura para pasar datos al hilo: ahora 2 sensores por hilo
typedef struct {
    TipoSensor sensor1;
    TipoSensor sensor2;
    int id;
} SensorArgs;

// Variable global compartida: conteo de alertas (protegida por mutex)
int alertas_totales = 0;
pthread_mutex_t mutex_alertas = PTHREAD_MUTEX_INITIALIZER;

// Archivo global para log
FILE *log_file;
pthread_mutex_t mutex_log = PTHREAD_MUTEX_INITIALIZER;

// Genera un número aleatorio en [min, max]
double rand_range(double min, double max) {
    return min + ((double)rand() / RAND_MAX) * (max - min);
}

// Verifica si un valor activa una alerta
int verificar_alerta(TipoSensor tipo, double valor) {
    switch (tipo) {
        case TEMPERATURA: return valor > 35.0;
        case HUMEDAD:     return valor < 30.0 || valor > 80.0;
        case RUIDO:       return valor > 90.0;
        case LUMINOSIDAD: return valor < 150.0;
        default:          return 0;
    }
}

// Obtiene el nombre del sensor
const char* nombre_sensor(TipoSensor tipo) {
    switch (tipo) {
        case TEMPERATURA: return "Temperatura";
        case HUMEDAD:     return "Humedad";
        case RUIDO:       return "Ruido";
        case LUMINOSIDAD: return "Luminosidad";
        default:          return "Desconocido";
    }
}

// Función del hilo: ahora procesa DOS sensores
void* hilo_sensor(void* arg) {
    SensorArgs* args = (SensorArgs*)arg;
    int id = args->id;
    TipoSensor s1 = args->sensor1;
    TipoSensor s2 = args->sensor2;

    int alertas_s1 = 0, alertas_s2 = 0;

    // Procesar primer sensor
    for (int i = 0; i < LECTURAS_POR_SENSOR; i++) {
        double valor;
        if (s1 == TEMPERATURA)      valor = rand_range(15.0, 45.0);
        else if (s1 == HUMEDAD)     valor = rand_range(20.0, 95.0);
        else if (s1 == RUIDO)       valor = rand_range(30.0, 120.0);
        else if (s1 == LUMINOSIDAD) valor = rand_range(100.0, 1000.0);
        else valor = 0;

        if (verificar_alerta(s1, valor)) {
            alertas_s1++;

            struct timespec ts;
            clock_gettime(CLOCK_REALTIME, &ts);

            pthread_mutex_lock(&mutex_log);
            fprintf(log_file, "[ALERTA] %s (Hilo %d): %.2f @ %ld.%09ld\n",
                    nombre_sensor(s1), id, valor, ts.tv_sec, ts.tv_nsec);
            fflush(log_file);
            pthread_mutex_unlock(&mutex_log);
        }
        usleep(100);
    }

    // Procesar segundo sensor
    for (int i = 0; i < LECTURAS_POR_SENSOR; i++) {
        double valor;
        if (s2 == TEMPERATURA)      valor = rand_range(15.0, 45.0);
        else if (s2 == HUMEDAD)     valor = rand_range(20.0, 95.0);
        else if (s2 == RUIDO)       valor = rand_range(30.0, 120.0);
        else if (s2 == LUMINOSIDAD) valor = rand_range(100.0, 1000.0);
        else valor = 0;

        if (verificar_alerta(s2, valor)) {
            alertas_s2++;

            struct timespec ts;
            clock_gettime(CLOCK_REALTIME, &ts);

            pthread_mutex_lock(&mutex_log);
            fprintf(log_file, "[ALERTA] %s (Hilo %d): %.2f @ %ld.%09ld\n",
                    nombre_sensor(s2), id, valor, ts.tv_sec, ts.tv_nsec);
            fflush(log_file);
            pthread_mutex_unlock(&mutex_log);
        }
        usleep(100);
    }

    // Actualizar total global
    pthread_mutex_lock(&mutex_alertas);
    alertas_totales += (alertas_s1 + alertas_s2);
    pthread_mutex_unlock(&mutex_alertas);

    // Imprimir resumen por hilo en consola
    printf("[Hilo %d] %s: %d alertas | %s: %d alertas\n",
           id,
           nombre_sensor(s1), alertas_s1,
           nombre_sensor(s2), alertas_s2);

    return NULL;
}

// Versión secuencial (igual que antes: 4 sensores uno tras otro)
void ejecutar_secuencial() {
    srand(12345); // Semilla fija

    FILE *log_sec = fopen("alertas_secuencial.log", "w");
    if (!log_sec) {
        perror("No se pudo crear alertas_secuencial.log");
        return;
    }

    int alertas_totales_sec = 0;
    TipoSensor tipos[] = {TEMPERATURA, HUMEDAD, RUIDO, LUMINOSIDAD};

    for (int s = 0; s < 4; s++) {
        int alertas_sensor = 0;
        for (int i = 0; i < LECTURAS_POR_SENSOR; i++) {
            double valor;
            switch (tipos[s]) {
                case TEMPERATURA: valor = rand_range(15.0, 45.0); break;
                case HUMEDAD:     valor = rand_range(20.0, 95.0); break;
                case RUIDO:       valor = rand_range(30.0, 120.0); break;
                case LUMINOSIDAD: valor = rand_range(100.0, 1000.0); break;
                default: valor = 0;
            }
            if (verificar_alerta(tipos[s], valor)) {
                alertas_sensor++;
                fprintf(log_sec, "[ALERTA SEC] %s: %.2f\n", nombre_sensor(tipos[s]), valor);
            }
            usleep(100);
        }
        alertas_totales_sec += alertas_sensor;
        printf("[Secuencial] %s finalizado. Alertas: %d\n", nombre_sensor(tipos[s]), alertas_sensor);
    }

    fclose(log_sec);
    printf("\n[Secuencial] Alertas totales: %d\n", alertas_totales_sec);
}

// Función principal
int main(int argc, char *argv[]) {
    srand(time(NULL));

    log_file = fopen("alertas.log", "w");
    if (!log_file) {
        perror("No se pudo crear alertas.log");
        return 1;
    }

    struct timespec inicio, fin;
    double tiempo_paralelo, tiempo_secuencial;

    // === Versión secuencial ===
    printf("Ejecutando versión secuencial...\n");
    clock_gettime(CLOCK_MONOTONIC, &inicio);
    ejecutar_secuencial();
    clock_gettime(CLOCK_MONOTONIC, &fin);
    tiempo_secuencial = (fin.tv_sec - inicio.tv_sec) + (fin.tv_nsec - inicio.tv_nsec) / 1e9;

    // Reiniciar para paralelo
    srand(time(NULL));
    fclose(log_file);
    log_file = fopen("alertas.log", "w");
    alertas_totales = 0; // Reiniciar contador

    // === Versión paralela con 2 hilos ===
    printf("\nEjecutando versión paralela (2 hilos, 2 sensores por hilo)...\n");
    clock_gettime(CLOCK_MONOTONIC, &inicio);

    pthread_t threads[NUM_HILOS];
    SensorArgs args[NUM_HILOS] = {
        {TEMPERATURA, HUMEDAD,     0},
        {RUIDO,       LUMINOSIDAD, 1}
    };

    for (int i = 0; i < NUM_HILOS; i++) {
        pthread_create(&threads[i], NULL, hilo_sensor, &args[i]);
    }
    for (int i = 0; i < NUM_HILOS; i++) {
        pthread_join(threads[i], NULL);
    }

    clock_gettime(CLOCK_MONOTONIC, &fin);
    tiempo_paralelo = (fin.tv_sec - inicio.tv_sec) + (fin.tv_nsec - inicio.tv_nsec) / 1e9;

    // === Resultados finales ===
    printf("==================================================\n");
    printf("RESULTADOS DE TIEMPO:\n");
    printf("Secuencial: %.4f segundos\n", tiempo_secuencial);
    printf("Paralelo:   %.4f segundos\n", tiempo_paralelo);
    printf("Aceleración: %.2fx\n", tiempo_secuencial / tiempo_paralelo);
    printf("Eficiencia: %.2f%%\n", (tiempo_secuencial / tiempo_paralelo) / NUM_HILOS * 100);
    printf("Alertas totales (paralelo): %d\n", alertas_totales);
    printf("==================================================\n");

    fclose(log_file);
    pthread_mutex_destroy(&mutex_alertas);
    pthread_mutex_destroy(&mutex_log);

    return 0;
}
